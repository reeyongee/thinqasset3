...
const cardEase =
  "transition-[border-radius,background-color,min-height,border-color] duration-500 ease-[cubic-bezier(0.12,0.23,0.17,0.99)]";
...
export function ApproachStepCard({
  ...
  if (!isOpen) {
    return (
      <button
        type="button"
        className={[
          "approach-step-card flex min-h-[73px] w-full cursor-pointer items-center overflow-hidden rounded-[80px] border border-solid border-[color:var(--token-btn-border)] bg-[var(--token-btn-bg)] px-6 py-6 text-left hover:bg-[var(--token-btn-bg-hover)] min-[1200px]:min-h-[76px]",
          cardEase,
        ].join(" ")}
        aria-expanded={false}
  ...
  return (
    <div
      className={[
        "approach-step-card min-h-[473px] w-full overflow-hidden rounded-2xl border border-solid border-[color:var(--token-btn-border)] bg-token-surface min-[1200px]:min-h-[411px] min-[1200px]:rounded-[32px]",
        cardEase,
      ].join(" ")}
      id={`approach-tab-${step.id}`}
    >
        ...
        <div className="relative z-[2] flex min-h-[473px] w-full flex-col px-6 py-6 min-[1200px]:min-h-[411px] min-[1200px]:max-w-[min(100%,420px)] min-[1200px]:px-8 min-[1200px]:py-6">
          ...
          >
            <div className="flex flex-col gap-4">
              ...
              <TransitionLink
                href={step.href}
                className="inline-flex py-1 font-[family-name:var(--font-inter)] text-sm font-medium text-ta-gold min-[1200px]:text-base"
              >
                {step.linkLabel} →
              </TransitionLink>
            </div>
